...
ArrayList<CommentDTO> carr = cmdao.semiListComment(cdto.getIdx());
...
<div id="content_box">
	...
	<div id="content_main">  
		...
		%>
			...
			if(id.equals(yid)){
				%>
			<div id="dot"><a href="remove_ok.jsp?idx=<%=cdto.getIdx()%>"><img src="/instargram/icon/remove.png"></a></div>
			<div id="dot"><a href="modify.jsp?idx=<%=cdto.getIdx()%>"><img src="/instargram/icon/modify.png"></a></div>
		...
		if(check_like==true){
			%>
		<img id="like" src="./icon/like_color.png" onclick="changeLike()">
			<%
		}else{
			%>
		<img id="like" src="./icon/like.png" onclick="changeLike()">
			<%
		}
		%>
		...
		if(check_tag==true){
			%>
		<img id="tag" src="./icon/tag_color.png" onclick="changeTag()">
			<%
		}else{
			%>
		<img id="tag" src="./icon/tag.png" onclick="changeTag()">
			<%
		}
		%>
		...
		<div id="content_text" style="margin-bottom: 10px;"> 
			<a href="#"><span style="font-weight:bold;"><%=cdto.getNickname() %></span></a>&nbsp;<span><%=cdto.getContent() %></span> 
		</div>
		<div id="reply">
			<%
			...
			<div id="past_time" style="margin:0px;">
				<%=carr.get(j).getRdate() %> 
			</div>
			<%
				}
		...
		<div id="past_time">
			<a href="#"><%=cdto.getRdate() %></a>
		</div>
		<div id="reply_input">
			<input type="hidden" value="<%=cdto.getIdx() %>" name="idx">
			<input type="hidden" value="<%=cdto.getId() %>" name="yid">
			<input type="hidden" value="<%=id %>" name="id">
			<input type="hidden" value="<%=nickname %>" name="nickname">
			
			<input type="text" id="reply_text" placeholder="댓글달기.." name="r_comment" onclick="input_check()" class="reply_text">
		   	<input type="submit" value="게시" id="reply_ok" disabled="disabled" class="reply_ok">
